...
const UserPicker = <T extends SelectableWithId = User>({
  ...
  const isUserSelected = (user: User) =>
    nonGuestSelection.some((selected) => (selected as unknown as User).id === user.id)
  ...
  const toggleUser = (user: User) => {
    ...
    const selectedUserItems = nonGuestSelection as unknown as User[]
  ...
  const removeSelectedItem = (item: T) => {
    ...
    emitUserSelectionChange(
      (nonGuestSelection as unknown as User[]).filter(
