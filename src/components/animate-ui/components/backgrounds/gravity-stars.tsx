...
function GravityStarsBackground({
  ...
  const previousCanvasSizeRef = React.useRef({ width: 0, height: 0 });
  ...
  const resizeStars = React.useCallback((w: number, h: number) => {
    const previous = previousCanvasSizeRef.current;
    const scaleX = previous.width > 0 ? w / previous.width : 1;
    const scaleY = previous.height > 0 ? h / previous.height : 1;

    starsRef.current.forEach((p) => {
      p.x = Math.min(w, Math.max(0, p.x * scaleX));
      p.y = Math.min(h, Math.max(0, p.y * scaleY));
    });
  }, []);
