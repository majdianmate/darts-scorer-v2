import {
  ...
  async function hydrateClubMembers(
    raw: Array<ClubMemberDoc & { id: string }>,
  ): Promise<ClubMember[]> {
    const userIds = [
      ...raw.flatMap((member) => [
        member.userId,
    ...
    return raw.map((member) => ({
      ...
      user: member.userId
        ? (userMap.get(member.userId) ??
          createGuestDisplayUser(member.guestName ?? "Guest", member.joinCode))
        : createGuestDisplayUser(member.guestName ?? "Guest", member.joinCode),
      invitedBy: member.invitedById
        ? (userMap.get(member.invitedById) ?? null)
        : null,
    }));
  }

  async function fetchClubMembersByIds(
    membershipIds: string[],
  ): Promise<Map<string, ClubMember>> {
    const unique = [...new Set(membershipIds.filter(Boolean))];
    if (!unique.length) return new Map();

    const memberSnaps = await Promise.all(
      unique.map((membershipId) => getDoc(doc(db, "clubMembers", membershipId))),
    );

    const raw = memberSnaps
      .filter((snap) => snap.exists())
      .map((snap) => ({
        id: snap.id,
        ...(snap.data() as ClubMemberDoc),
      }));

    const members = await hydrateClubMembers(raw);
  ...
  async function fetchSquadMembers(
    ...
    const memberMap = await fetchClubMembersByIds(
      raw.map((squadMember) => squadMember.membershipId || squadMember.memberId),
    );

    return raw.map((squadMember) => {
      const membershipId = squadMember.membershipId || squadMember.memberId;
      const member = memberMap.get(membershipId);

      if (!member) {
        throw new Error(`Club member not found: ${membershipId}`);
      }
  ...
  export async function addClubMemberService(
    ...
    const safeClubId = assertId(clubId, "Club ID");
    const safeInvitedById = assertId(invitedById, "Inviter ID");
    const guestName = target.guestName?.trim();
    if (guestName) {
      await addClubGuestService(safeClubId, safeInvitedById, guestName);
      return;
    }
    ...
    const memberId = `${safeClubId}_${target.userId}`;
  ...
  export async function createSquadService(
    ...
    const safeClubId = assertId(clubId, "Club ID");
    const createdById = assertId(createdBy.id, "Creator ID");
    const batch = writeBatch(db);
    const limitedMembers = data.members?.slice(0, 5) ?? [];
    const memberIds = limitedMembers.map((member) => member.id);
    ...
    if (limitedMembers.length > 0) {
      limitedMembers.forEach((member) => {
        ...
        const squadMember: SquadMemberDoc = {
          squadId,
          clubId: safeClubId,
          memberId: member.userId ?? member.id,
