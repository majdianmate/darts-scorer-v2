...
const MemberManagerDialog = () => {
  const open = useStore((state) => state.dialogs['memberManager'])
  const targetClubId = useStore((state) => state.targetClubId)
  const setDialogOpen = useStore((state) => state.setDialogOpen)
  const { club, addMemberAsync, addGuestAsync, isAddMemberPending } = useClub(targetClubId ?? '');
  ...
  const handleOpenChange = (open: boolean) => {
    setDialogOpen('memberManager', open)
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => setDialogOpen('memberManager', open)}>
      <DialogContent className="flex max-h-[min(720px,calc(100vh-2rem))] flex-col gap-0 overflow-hidden p-0 sm:max-w-5xl xl:max-w-6xl">
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden sm:flex-row sm:min-h-[400px]">
          <div className="flex min-h-0 min-w-0 flex-[1.15] flex-col overflow-hidden border-b border-border sm:border-b-0">
