...
function statusVariant(status: MatchStatus): "default" | "secondary" | "outline" {
  switch (status) {
...
const MatchListItem: FC<MatchListItemProps> = ({ match, clubName }) => {
        ...
        <CardHeader className="flex flex-row items-start justify-between gap-3">
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <CardDescription className="line-clamp-2">
              {clubName ? `${clubName} · ` : ""}
              {match.matchConfig.startingScore} · {match.matchConfig.numberOfLegs}{" "}
              legs · leg {match.currentLeg}
            </CardDescription>
          </div>
